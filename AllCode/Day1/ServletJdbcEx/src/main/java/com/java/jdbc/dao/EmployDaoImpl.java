...
public class EmployDaoImpl implements EmployDao {
	...
	@Override
	public String updateEmployDao(Employ employUpdate) throws ClassNotFoundException, SQLException {
		Employ employ = searchEmployDao(employUpdate.getEmpno());
		if (employ ==null) {
			return "Employ Record Not Found...";
		}
		connection = ConnectionHelper.getConnection();
		String cmd = "Update Employ set Name = ?, Gender = ?, Dept = ?, Desig = ?, "
				+ " Basic = ? WHERE Empno = ?";
		psmt = connection.prepareStatement(cmd);
		psmt.setString(1, employUpdate.getName());
		psmt.setString(2, employUpdate.getGender().toString());
		psmt.setString(3, employUpdate.getDept());
		psmt.setString(4, employUpdate.getDesig());
		psmt.setDouble(5, employUpdate.getBasic());
		psmt.setInt(6, employUpdate.getEmpno());
		psmt.executeUpdate();
