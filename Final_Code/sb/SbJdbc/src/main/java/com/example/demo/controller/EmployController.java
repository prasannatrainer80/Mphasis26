...
public class EmployController {
    ...
    //http://localhost:9000/showByGender/FEMALE
    
    @GetMapping("/showByGender/{gender}")
    public ResponseEntity<List<Employ>> 
    	showByGender(@PathVariable String gender) {
        return ResponseEntity.
        		ok(employService.showByGender(gender));
    }
    
    //http://localhost:9000/showByDept/JAVA
    
    @GetMapping("/showByDept/{dept}")
    public ResponseEntity<List<Employ>> 
    	showByDept(@PathVariable String dept) {
        return ResponseEntity.
        		ok(employService.showByDept(dept));
    }
    
    //http://localhost:9000/showByDeptAndGender/JAVA/MALE
    @GetMapping("/showByDeptAndGender/{dept}/{gender}")
    public ResponseEntity<List<Employ>> 
	showByDeptAndGender(@PathVariable String dept,
			@PathVariable String gender
			) {
    return ResponseEntity.
    		ok(employService.showByDeptAndGender(dept, gender));
} 
}
